...
## 4. Transfer Family セキュリティグループ

| SG ID | SG名 | 対象 |
|---|---|---|
| sg-06153ac3ff38765ab | ksm-posstg-vpc-sg-ep-tf | 全Transfer Family VPC EP（3台共通） |

**インバウンドルール:**

| プロトコル | 送信元 | 説明 | 評価 |
|---|---|---|---|
| TCP 22 | 10.156.96.192/26 | For SFTP Inbound | ✅ 正常 |
| TCP 22 | sg-01f1bbc2ae66a6591（Bastion SG） | test for bastion | 🔴 **テスト残骸・要削除** |

**PRDとの差異:** STGのみBastionからTCP22が許可されている。`test for bastion`という説明からテスト目的で追加されたまま残存。**削除が必要。**
...
| web-be SG | 🔴 **全通信許可(-1)** |
| Transfer Family SG（Bastion許可） | 🔴 **テスト残骸ルール残存（要削除）** |
