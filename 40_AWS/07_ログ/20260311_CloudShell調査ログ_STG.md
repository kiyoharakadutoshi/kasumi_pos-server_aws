...
## [H] DB詳細調査結果（2026-03-11）

### 42_P003 テーブル状態

| テーブル | 行数 | data_mb | index_mb | 合計 |
|---|---|---|---|---|
| 42_P003 | 2,606,064 | 295MB | 164MB | 459MB |
| **42_P003_history** | **106,764,454** | **18,691MB** | **14,181MB** | **32,872MB（約32GB）** 🚨 |

**42_P003_history が約1億行・32GB → P003.csv取込時にhistoryへのINSERTが異常に遅くなる根本原因**

### Replica_Kasumi 巨大テーブル TOP10

| テーブル | 行数 | size_mb |
|---|---|---|
| **42_P003_history** | **106,764,454** | **32,872** |
| 41_P001_history | 14,875,312 | 27,982 |
| 41_P001 | 12,471,014 | 17,833 |
| 01_GHPLUM_history | 12,696,321 | 15,716 |
| 01_GHPLUM | 10,079,651 | 11,721 |
| 02_GHDISCOUNTM | 14,493,212 | 6,798 |
| 13_SpecialPriceKsmMaster_history | 5,023,030 | 2,258 |
| 11_ItemMaster_history | 496,503 | 1,513 |
| 82_ESLDATA | 2,464,479 | 1,393 |
| ESLDATA | 6,334,115 | 851 |

**historyテーブル群が肥大化している（削除・アーカイブ未実施）**

### 取込状況テーブル直近レコード

- 2026-03-11 02:06 343店舗 PLUマスタ・特売マスタ取込 → 正常
- 2026-03-11 01:53 218店舗 PLUマスタ・特売マスタ取込 → 正常
- 2026-03-11 01:54 253店舗 PLUマスタ・特売マスタ取込 → 正常
- 2026-03-10 ESL取込（218店舗）→ 正常
- **SH（P003）の取込状況レコードが見当たらない** → 取込状況テーブルへの書き込み前にタイムアウトしている

### ProcedureLog

- 最新レコードが 2025-08-21（約7ヶ月前）→ ESL export処理のみ記録

---

### 🎯 障害① 根本原因 完全確定

**`42_P003_history` テーブルが 1億行・32GB まで肥大化しており、P003.csv（約75MB・264万行）の取込時に `42_P003_history` へのINSERTが900秒以内に完了しない。**

**メカニズム:**
1. import-pos-master-sh 起動
2. `stg/Replica_Kasumi` に接続
3. `棚番情報_TMP` テーブル CREATE（成功）
4. `42_P003` の既存データを `42_P003_history` にINSERT（**約264万行 → 32GB超のテーブルへの追記**）
5. この処理が900秒以内に完了せず Lambda タイムアウト

**解決策:**
- 短期: `42_P003_history` の古いレコードをDELETE（例: 3ヶ月以上前）してテーブルを縮小
