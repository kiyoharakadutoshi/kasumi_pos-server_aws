...
## 29. 運用コスト妥当性チェック（2026-03-11）

> 詳細: `20260311_コスト設定妥当性チェック.md`

### 月額コスト概要（2025-09〜2026-02 実績）
...
### サービス別コスト構成（6ヶ月合計 $22,654）

| サービス | 金額 | 構成比 | 妥当性 |
|---|---|---|---|
| RDS（Aurora） | $13,564 | 59.9% | ❌ **非妥当**（PRD同様に4〜8倍過剰スペック） |
| Transfer Family | $3,325 | 14.7% | ⚠️ やや非妥当（PRD同様に統合で削減可能） |
| EC2 | $1,835 | 8.1% | ✅ 妥当（ただしPRDより$574高い＝web-be/web-fe 2台分） |
| VPC | $1,069 | 4.7% | ✅ 妥当（ALB用ENI分がPRDより高い） |
| EC2その他 | $455 | 2.0% | ✅ 妥当 |
| S3 | $145 | 0.6% | ⚠️ 2月に$72急増（PRDは$5）→ P003_history肥大化関連の可能性 |
| ALB | $85 | 0.4% | ✅ 妥当（PRDには存在しない） |
| その他 | $176 | 0.8% | ✅ 妥当 |
...
| EC2が4台（PRDは2台） | +$96/月 |
| ALBが2台（PRDは0台） | +$14/月 |
| S3急増（2月: $72 vs PRD $5） | +$67/月 |
| VPC（ALB用ENI追加） | +$44/月 |
| セキュリティサービス無効 | -$74/月（本来はコスト増で有効化すべき） |

### ❌ STG固有の問題

| 問題 | 影響 | 対応 |
|---|---|---|
| GuardDuty/SecurityHub/Inspector 全無効 | 脅威検知なし。PRDへの踏み台リスク | aws_012（改修待ち） |
| 42_P003_history 1億行32GB肥大化 | SF障害中 + S3バックアップコスト増 | **aws_021（最優先）** |
| web-be SG ALL開放 | 全通信許可のまま | aws_018（改修No.5） |

### 削減計画と見込み額

| 施策 | ドキュメント | 月削減額 | 年間削減額 | ステータス |
|---|---|---|---|---|
| RDS r5.2xlarge→r5.xlarge | aws_006 | $1,004 | **$12,048** | 承認待ち |
| Transfer Family 3台→1台統合 | aws_007 | $432 | **$5,184** | 承認待ち |
| P003_history縮小（S3コスト正常化） | aws_021 | $67（推定） | $804 | **最優先** |
| **合計** | | **$1,503** | **$18,036** | |
