...
import { NormalIconButton } from '@/components/button/flat-button/flat-button';
import { KeyboardViewContext } from '@/components/keyboard-navigation/keyboard-navigation';
...
const ExcludedStore = ({
  ...
  const [tabs, setTabs] = useState<SideBarTab[]>([
    {
      name: 'label.excludedStores',
      type: SideBarTabType.Store,
    },
  ]);
  const [indexTab] = useState(0);
  ...
  return (
    <div className="exclude-store__store">
      ...
      <div className="exclude-store__store-content">
        <ul
          className={`list-unstyled exclude-store__list-store ${selectMultiple
