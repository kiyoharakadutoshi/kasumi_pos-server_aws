...
import {
  ...
  setSelectedStore,
  updateSelectedStore,
} from 'app/reducers/store-reducer';
...
const SidebarStoreDefault = ({
  ...
  /*
    Fetch list store when re render side bar and
    handle focus first store selected
  */
  useEffect(() => {
    dispatch(getListStore())
      .unwrap()
      .then((res) => {
        const fetchedStores = res?.data?.data?.items;
        // Handle focus first store selected ( radio)
        if (storeRefs.current?.length !== fetchedStores?.length) {
          storeRefs.current = Array(fetchedStores?.length)
            .fill(null)
            .map(() => React.createRef<HTMLLIElement>());
        }

        if (selectMultiple || isNullOrEmpty(fetchedStores)) {
          return;
        }

        if (fetchedStores.some((store) => store.selected)) {
          return;
        }
  ...
  return (
    <div className="sidebar-store">
      ...
      <div className={`sidebar-store__container ${expanded ? 'sidebar-store__expanded' : ''}`}>
        <div className={`sidebar-store__tab-menu`} style={{ gridTemplateColumns: tabs.map(() => '1fr').join(' ') }}>
          {tabs.map((tab, index) => (
...
const MenuBarStore = ({
  ...
  const filteredStores = () => {
    ...
    return stores.filter(
      (store) =>
        `${store.store_code}：${store.short_name.toLowerCase()}`.includes(keywordSearch) &&
