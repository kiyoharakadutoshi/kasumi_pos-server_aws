...
const MasterCategoryModal = ({ setOpenModal }) => {
  ...
  const handleGetDetailDiscountCategory = (item: IMasterCategory) => {
    dispatch(
      getDetailDiscountCategory({
        selected_store: selectedStores[0],
        md_hierarchy_code: item?.md_hierarchy_code,
        md_hierarchy_level: item?.md_hierarchy_level,
    ...
    () => [
      {
        ...
        buttonInput: {
          ...
          onClick(row) {
            handleGetDetailDiscountCategory(row);
          },
        },
      },
      {
      ...
      },
      {
        ...
        option(props) {
          if (props.row.original.time_service === 0) {
            return {
              value: `${props.row.original.time_service} : 通常`,
            };
          } else if (props.row.original.time_service === 1) {
            return {
              value: `${props.row.original.time_service} : タイムサービス`,
            };
          }
        },
      },
