...
const ProductManagement = () => {
      ...
      case '01':
        return {
          value: item?.combination_options
            ?.map(
              (combination) =>
                `${formatNumber(combination.quantity)}個${formatNumber(combination.price, 1)}円(成立後${formatNumber(combination.one_price, 1)}円)`
            )
            ?.join(' | '),
        };
      case '02':
        return {
          value: item?.combination_options
            ?.map((combination) => `${formatNumber(combination.quantity)}個${formatNumber(combination.price, 1)}%`)
            ?.join(' | '),
        };
      case '03':
        return {
          value: item?.combination_options
            ?.map((combination) => `${formatNumber(combination.quantity)}個${formatNumber(combination.price, 1)}円`)
            ?.join(' | '),
        };
      default:
        return { value: '' };
  ...
  const handleDataProduct = (response: AxiosResponse<IProductResponse, any>, isPlu?: boolean) => {
    ...
    if (response.data?.data?.item_code) {
      ...
      dispatch(getListMixMatchsSpecialPrice({ plu: item_code, selected_store: selectedStore }));
    } else {
  ...
  const navigateToSC0301 = (promotionCode?: string) => {
    ...
    setSelectRow();
    navigate(URL_MAPPING.SC0301, { state: dataSC0301 });
  };
  ...
  const setSelectRow = () => {
    // Save selected rows to redux store, remove function in TanStack row when saving to redux to avoid warning
    dispatch(setSelectedRows(getValues('selectedRows')?.map((row) => stripFunctions(row))));
  };
