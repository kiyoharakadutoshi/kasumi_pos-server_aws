...
import {
  END_HOURS_DEFAULT,
  GROUP_PRODUCT_CODE_INPUT_MAX_LENGTH,
  KEYDOWN,
  START_HOURS_DEFAULT,
  STATUS,
...
import { focusElement, focusInputElementTable, handleFocusListElement } from 'app/helpers/utils-element-html';
...
const SpecialPromotionEdit = () => {
  ...
  useEffect(() => {
    ...
    if (store) {
      ...
      dispatch(setApplyInfo({ key: 'start_time', value: START_HOURS_DEFAULT }));
      dispatch(setApplyInfo({ key: 'end_time', value: END_HOURS_DEFAULT }));
    }
...
const SearchSpecialPromotion = forwardRef<ISearchSpecialPromotionHandle, ISearchPromotionView>(
  ({ isStandardPrice, setIsStandardPrice, isPercentPrice, setIsPercentPrice, enabledPLU }, ref) => {
    ...
    /**
     * When suggested promotion or select promotion in modal search promotion -> focus price input
     */
    useEffect(() => {
      if (suggestedPromotion?.hasData === true) {
        const specialPrice: SpecialPromotionKey = 'special_price';
        const discountValue: SpecialPromotionKey = 'discount_value';
        focusInputElementTable(null, true, null, isPercentPrice ? discountValue : specialPrice);
      }
    }, [suggestedPromotion?.hasData]);

    /**
     * 1.Action tab, enter in mode edit
     * 2. Stop tab, enter common in mode add
     */
    useEffect(() => {
      const nextElements = [];
      const prevElements = [];
      ...
      const statusElement = document.querySelector('.status .tooltip-number-input-text');
      if (statusElement) focusElements.push(statusElement);

      const elementPrice = document.querySelector(
        '.special_price .tooltip-number-input-text, .discount_value .tooltip-number-input-text'
      );

      if (elementPrice) {
        nextElements.push(elementPrice);
        prevElements.push(elementPrice);
        focusElements.push(elementPrice);
      }

      const elementTime = document.querySelector('.end_time .time-picker__time-picker-input');
      if (elementTime) nextElements.push(elementTime);

      const typeCodeElement = document.querySelector('.type_code .tooltip-number-input-text');
      if (typeCodeElement) prevElements.push(typeCodeElement);

      const onKeydown = (e: KeyboardEvent) => {
        if (e.key === KEYDOWN.Tab || e.key === KEYDOWN.Enter) {
          // Check mode edit
          if (isEditMode) {
            actionFocusModeEdit(focusElements, elementTime, e);
            return;
          }

          // Shift-Tab in type code input
          if (e.shiftKey) {
            if (prevElements.includes(document.activeElement)) {
              e.preventDefault();
              e.stopPropagation();
            }
            return;
          ...
          // Tab, enter in special_price or end time picker
          if (nextElements.includes(document.activeElement)) {
            e.preventDefault();
            e.stopPropagation();
            return;
          }
    ...
    /**
     * Action focus in mode Edit
     * @param focusElements
     * @param timeElement
     * @param event
     */
    const actionFocusModeEdit = (focusElements: any[], timeElement: Element, event: any) => {
      // Focus between status and price
      if (focusElements?.includes(document.activeElement)) {
        handleFocusListElement(focusElements, event);
        event.preventDefault();
        event.stopPropagation();
        return;
      }

      // Check next focus in end time
      if (document.activeElement === timeElement && !event.shiftKey) {
