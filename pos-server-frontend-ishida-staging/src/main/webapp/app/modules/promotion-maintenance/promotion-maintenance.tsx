...
import { END_HOURS_DEFAULT, START_HOURS_DEFAULT, USER_ROLE } from '@/constants/constants';
import { getListPromotion, IListPromotionSearch, updateListPromotionMaintenance } from 'app/services/promotion-service';
...
const LIMIT_RECORD_TABLE = 13;

const START_WITH_PROMOTION_CODE = '99';

const CONFIRM_ACTION_STATUS = -1;
...
const promotionMaintenanceListDefault: IPromotionMaintenanceList = {
  ...
  currentPage: 1,
  promotionCode: '',
  dataList: repeatObject(LIMIT_RECORD_TABLE),
  isDirty: false,
  disabledClear: true,
};
...
const PromotionMaintenance = () => {
  ...
  const setValueColumn = (value: string, index: number) => {
    ...
    if (getValues('dataList')[index].record_idx === null) {
      ...
      setValue(`dataList[${index}].start_time` as any, START_HOURS_DEFAULT);
      setValue(`dataList[${index}].end_time` as any, END_HOURS_DEFAULT);
    }
  };

  /**
   * Checks if the given promotion code starts with "99".
   *
   * @param {string} promotionCode The promotion code.
   * @returns {boolean} Return `true` if the code starts with "99", otherwisr `false`.
   */
  const checkPromotionCodeStartWith99 = (promotionCode: string): boolean => {
    return promotionCode.startsWith(START_WITH_PROMOTION_CODE);
  };
  ...
  const validatePromotionCode = (promotionCode: string, index: number): boolean => {
    ...
    if (checkPromotionCodeStartWith99(promotionCode)) {
      setError(`dataList[${index}].promotion_code` as any, null);
      return true;
    }
  ...
  const validateDateTime = (index: number): boolean => {
    ...
    const startDateStamp = Date.parse(start_date);
    const endDateStamp = Date.parse(end_date);
    ...
    if (start_time === 'Invalid Time' || end_time === 'Invalid Time') {
      return false;
    }

    if (startDateStamp === endDateStamp) {
      const startTime = timeToMinutes(start_time);
      const endTime = timeToMinutes(end_time);
  ...
  const handleSearchPromotion = (pageNumber: number) => {
    clearErrors();
    const query: IListPromotionSearch = {
      selected_store: selectedStores[0],
      page_number: pageNumber,
      promotion_code: getValues('promotionCode') ?? null,
      limit: LIMIT_RECORD_TABLE,
    };
    if (isNullOrEmpty(query.promotion_code)) delete query.promotion_code;
    ...
    dispatch(getListPromotion(query))
      ...
      .then((res) => {
        ...
        const promotions = res.data?.data?.items;
        let dataList = promotions?.map((item) => {
          ...
          return {
            ...
            promotion_cate: getPromotionTypeText(_.toString(item.type)),
        ...
        const pad = dataList?.length === undefined ? LIMIT_RECORD_TABLE : LIMIT_RECORD_TABLE - dataList?.length;
        if (pad > 0) {
  ...
  const actionPaging = (pagingAction?: 'next' | 'prev' | 'first' | 'last') => {
    ...
    handleSearchPromotion(redirectPage);
  };
  ...
  const updatePromotionMaintenanceList = (redirectPage: number) => {
    ...
    const element = getValues('dataList');

    for (let i = 0; i < LIMIT_RECORD_TABLE; i++) {
      const item = element[i];
      const isValidPromotionCode = validatePromotionCode(item?.promotion_code, i);

      if (!isValidPromotionCode) {
        if (isNull(item?.record_idx) && item?.promotion_code) {
          haveErrs = true;
        }
        continue;
      }
    ...
    return dispatch(updateListPromotionMaintenance(requestValue))
      .then((response) => {
        if ('Success' === response.payload?.data?.status) {
          if (redirectPage === CONFIRM_ACTION_STATUS) {
            clearData();
          } else {
            setValue('currentPage', redirectPage);
            handleSearchPromotion(watch('currentPage'));
          }
        } else {
          ...
          for (const [key, value] of Object.entries(errs)) {
            if (Object.prototype.hasOwnProperty.call(errs, key)) {
              ...
              errsString =
                ...
                ' : ' +
                localizeFormat(value[0], 'promotionMaintenance.promotion_name') +
                '\n';
            }
          }
      ...
      <div className="promotion-maintenance__container">
        ...
        <PagingBottomButtonControl
          ...
          confirmAction={() => {
            formConfig
              .handleSubmit(() => {
                updatePromotionMaintenanceList(CONFIRM_ACTION_STATUS);
              })()
              .then(() => {
                for (let i = 0; i < LIMIT_RECORD_TABLE; i++) {
