...
export const ProductReport = () => {
  ...
  const [maxLengthNumber, setMaxLengthNumber] = React.useState(2);
  ...
  const { getValues, setValue, setError, reset, watch, clearErrors, resetField } = formConfig;
  ...
  const handleChangeExportUnit = () => {
    setValue('searchCondition.level', 1);
    resetField('searchCondition.codeLevel');
    resetField('searchCondition.pluCode');
    resetField('searchCondition.pluCodeName');
    resetField('searchCondition.productCode');
    resetField('searchCondition.productCodeName');
    resetField('searchCondition.codeLevelName');
  };
      ...
      .then((res) => {
        ...
        if (isShowExportByCategory) {
          if (hierarchyLevelData.total_count === 0) {
            ...
            clearErrors('searchCondition.codeLevelName');
            setValue('searchCondition.codeLevelName', hierarchyLevelData.items[0].description);
          }
        } else {
          if (hierarchyLevelData.total_count === 0) {
            ...
            clearErrors('searchCondition.productCodeName');
            setValue('searchCondition.productCodeName', hierarchyLevelData.items[0].description);
          }
        }
      })
      .catch(() => {});
  ...
  const handleSetMaxLength = (level: number | string) => {
    if (isInageya) {
      const setLevelMethod = {
        1: () => setMaxLengthNumber(2),
        2: () => setMaxLengthNumber(4),
        3: () => setMaxLengthNumber(6),
        4: () => setMaxLengthNumber(6),
      };
      return setLevelMethod[level]();
    }

    const setLevelMethod = {
      1: () => setMaxLengthNumber(2),
      2: () => setMaxLengthNumber(5),
      3: () => setMaxLengthNumber(5),
      4: () => setMaxLengthNumber(5),
    };
    return setLevelMethod[level]?.();
  };

  const handleOnChangeLevel = (level: number | string) => {
    resetField('searchCondition.codeLevelName');
    handleSetMaxLength(level);
  };

  // Handle change store
  useEffect(() => {
    handleClearData();
    handleSetMaxLength(1);
  }, [selectedStores[0]]);
    ...
    <FormProvider {...formConfig}>
      <div className="product-report">
        <Header
          ...
          hiddenTextESC={true}
        />
        {openModal && <HierarchyLevelModal setOpenModal={setOpenModal} selectedItem={setSelectedItem} handleSetMaxLength={handleSetMaxLength} />}
              ...
              <div className={'product-report__search-section-1'}>
                <div className={`product-report__business-type left`}>
                  <div className={`product-report__business-type-radio`}>
                    <span className="product-report__label">
                ...
                <div className={`${isShowExportByCategory ? 'product-report__show-export-category' : 'hidden'}`}>
                  <div className={'product-report__category'}>
                    <Dropdown
                      ...
                      onChange={(item) => {
                        handleOnChangeLevel(item.value);
