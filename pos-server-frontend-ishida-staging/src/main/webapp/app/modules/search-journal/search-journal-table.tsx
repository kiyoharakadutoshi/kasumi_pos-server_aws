...
import { IsAscendingType } from './search-journal-interface';
import { IS_TRAINING_MODE, TYPE_JOURNAL_TABLE } from './constant/default-data';
...
const CellText = <TBody extends TBodyBase>({
  ...
  if (column.keyItem === 'type') {
    newValue = TYPE_JOURNAL_TABLE[newValue] || null;
  }

  if (column.keyItem === 'is_training_mode') {
    newValue = IS_TRAINING_MODE[newValue];
  }
  useLayoutEffect(() => {
                ...
                >
                  {typeof column.title === 'string' && column.title ? (
                    <div
                      ...
                      onClick={() => !columnNotSort.includes(column?.keyItem as any) && handleSortData(column)}
                    >
                      ...
                      {!columnNotSort.includes(column?.keyItem as any) && (
                        <div className="table-header-sort__icon">
                          <svg
                            style={{
                              transform:
                                currentSortColumn === column?.keyItem && isAscending[column?.keyItem] === true
                                  ? 'scaleY(-1)'
                                  : '',
                              opacity: currentSortColumn !== column?.keyItem && 0.3,
                            }}
                        ...
                        >
                          {(() => {
                            switch (column.type) {
                              ...
                              default:
                                return <CellText column={column} item={item} isNewRow={isNewRow} />;
                            }
                          })()}
                        </td>
                      );
