...
export const validateSpecialPromotions = (
  ...
  for (const item of items) {
    ...
    if (!isNullOrEmpty(item['start_date'])) {
      if (!isValid(startDate)) {
        ...
        if (startDateTime < minDateTime) {
          ...
          isError = true;
        } else if (startDateTime >= maxDateTime) {
          error['start_date'] = 'MSG_VAL_041';
          isError = true;
          if (error['end_date'] === 'MSG_VAL_041') error['end_date'] = null;
        }
      }
    }
    ...
    if (
      ...
      const validateError = item.type_code === '0' ? item.start_date === item.end_date : startDate <= endDate;
      if (validateError) {
