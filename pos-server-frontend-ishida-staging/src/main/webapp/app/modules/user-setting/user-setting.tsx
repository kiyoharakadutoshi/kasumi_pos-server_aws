...
import { formatValue, isNullOrEmpty, localizeString } from 'app/helpers/utils';
...
import { DEFAULT_VALUE, ListUserRequest, UserSaveCondition, UserTable } from "./interface-user";
...
const UserSetting = (): JSX.Element => {
  ...
  const handleEditUser = () => {
    ...
    const editData = {
      ...
      storeCode: getValues('edit.storeCode') || selectedRowData?.original?.store_code,
      storeName: getValues('edit.storeName') || selectedRowData?.original?.store_name,
      ...
      password: getValues('edit.password') || selectedRowData?.original?.['password'],
      confirmPassword: getValues('edit.confirmPassword') || selectedRowData?.original?.['confirm_password'],
    }
    ...
    () => [
      ...
      },
      {
        ...
        option(props, defaultValue) {
          ...
          return {
            value: formatValue(user?.store_code, user?.store_name),
            defaultValue: formatValue(defaultValue?.store_code, defaultValue?.store_name)
          };
        },
        type: 'text',
      },
      ...
      <FormProvider {...formConfig}>
        ...
        <main className="user-setting">
          ...
          <TableData<UserTable>
            ...
            rowConfig={(row) => {
              return {
                className: row?.original.operation_type === OperationType.Edit ? 'is_update' : ''
              };
            }}
          />
        </main>
