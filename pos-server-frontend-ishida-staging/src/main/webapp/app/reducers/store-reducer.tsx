...
import { CODE_MASTER, GROUP_STORE_MASTER_CODE, GROUP_STORES, STORE_USER } from 'app/constants/constants';
    ...
    builder
      .addCase(getListStore.fulfilled, (state, action) => {
        ...
        if (state.stores?.length === 1) {
          ...
          state.stores.forEach((store) => (store.selected = state.selectedStores?.includes(store.store_code)));
        }
      })
      .addCase(getMasters.fulfilled, (state, action) => {
        if (action.payload?.config?.data?.includes(CODE_MASTER.STORE_CODE)) {
          const groupStores: IDropDownItem[] = action.payload.data.data
            .find(item => item.master_code === CODE_MASTER.STORE_CODE)
            ?.items?.map(item => ({
