...
export interface CashChangerSettingResponse extends ResponseApi {
  data: CashChangerSetting
}

export const paymentMachineLayouts = getDataWithParam<{ search_name?: string }, any>(`cash-register-type`)
export const postCashRegister = getDataWithMethodPost<{ cashRegister?: CashRegister[] }, any>(`cash-register-type/maintenance`)
export const getCashRegisterDetail = getDataWithPath<{ store_code: string; code: string | number }, CashRegisterResponse>('cash-registers/');
...
export const maintenance = postData<{cash_registers: CashRegisterPayload[]}>('cash-registers/maintenance')
