...
const DatePickerCustom: React.FC<IDatePickerProps> = ({
  ...
  const isFutureDate = (date: Date | null) => date && date > new Date();
  ...
  const onBlur = (e: React.ChangeEvent<HTMLInputElement>) => {
    handleFormatAndSetDate(e.target.value.replace(/\//g, ''));
  };

  const handleChooseDate = () => {
    setIsCalendarVisible(false);
    handleFormatAndSetDate(dateString);
  };

  const handleFormatAndSetDate = (inputData: string) => {
    const dateInit = initValue ? new Date(initValue) : new Date();
    const inputValue = inputData.replace(/\//g, '');
  ...
  return (
    <DatePickerContainer style={{ cursor: isDisable && 'not-allowed' }} ref={pickerRef} className={className}>
      <div style={{ height: '100%' }} onClick={() => setIsCalendarVisible(true)}>
        <InputTextCustom
          heightInput={heightDateTime}
      ...
      {isCalendarVisible && (
        <DatePickerCalendar isVisible={isCalendarVisible && !isDisable} position={position} left={left}>
          <DatePickerHeader>
