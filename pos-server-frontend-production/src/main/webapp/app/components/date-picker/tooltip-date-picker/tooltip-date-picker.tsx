...
const TooltipDatePicker: React.FC<IDatePickerProps> = ({
  ...
  const handleKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.key === 'Tab' && isCalendarVisible) {
      event.stopPropagation();
      event.preventDefault();
      handleFocusListElement(getFocusableElements(calendarRef.current), event, true);
      return;
    }

    if (
      isCalendarVisible &&
      (event.key === 'ArrowDown' || event.key === 'ArrowUp' || event.key === 'ArrowRight' || event.key === 'ArrowLeft')
    ) {
      const inputElement: HTMLInputElement = pickerRef.current.querySelector('.input-text-mark');
  ...
  const handleEnterEvent = (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (isCalendarVisible && document.activeElement !== inputElement) {
      event.stopPropagation();
      inputElement.focus();
    ...
    if (isNullOrEmpty(inputElement.value)) {
      outFocus();
      return;
    }
    ...
    // Get index focus (year, month, day)
    const index = Math.floor(inputElement.selectionStart / 3);
    const values = dateString?.replace(/[^_0-9/]/g, '').split('/');

    // If index focus is day of week, empty date => out focus
    if (index >= values?.length) {
      outFocus();
      return;
    }

    // Check year, month or day value empty entered
    const value = values[index]?.replace(/[^0-9]/g, '');

    // Create new date string
    values[index] = isNullOrEmpty(value) ? '__' : value.padStart(2, '0');
    const newDateStr = values.join('/') + dateString.slice(-3);
    setDateString(newDateStr);

    // focus to new element (month, day)
    const nextPosition = index * 3 + 3;
    inputElement.setSelectionRange(nextPosition, nextPosition);

    // If focus is day => out focus
    if (index >= 2) {
      outFocus();
    } else {
      event.stopPropagation();
  ...
  const handleArrowKey = (type: 'ArrowUp' | 'ArrowDown' | 'ArrowLeft' | 'ArrowRight') => {
    ...
    if (isValidDateInRange(nextDate, minDate, maxDate)) {
