  ...
  const displayStoreName = () => {
    if (storeReducer?.stores.length > 1) {
      ...
      if (storesCurrently?.length === 1)
        return TruncateText(storesCurrently[0]?.store_code, storesCurrently[0]?.store_name);
      const selectedStoreCustom = props?.selectedStore?.split(':');
      return TruncateText(selectedStoreCustom?.[0], selectedStoreCustom?.[1]);
    } else {
      return TruncateText(storeReducer.stores[0]?.store_code, storeReducer.stores[0]?.store_name);
    }
  };
...
const ActionMode = ({ mode }: { mode: ActionModeType }) => {
  switch (mode) {
    case 'edit':
      return (
        <label className="header-container__action-mode header-container__mode-edit">
          <Translate contentKey="header.modeEdit" />
        </label>
      );
    case 'add':
      return (
        <label className="header-container__action-mode header-container__mode-add">
