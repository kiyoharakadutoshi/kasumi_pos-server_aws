...
import { hideLoading, showLoading } from '../loading/loading-reducer';
import { KEYDOWN } from 'app/constants/constants';

interface IPrinterCustomProps {
  disabled?: boolean;
  printData?: any;
}

const PrinterCustom: React.FC<IPrinterCustomProps> = ({ disabled, printData }) => {
  ...
  const actionPrint = () => {
    if (disabled) return;
    dispatch(showLoading('export data'));

    setTimeout(async () => {
      try {
        const { renderPDF } = await import('./render-pdf');
        const blob = await renderPDF(printData);
        saveAs(blob, 'ジャーナル検索.pdf');
      } catch (error) {
        console.log('Export Error', error);
        dispatch(hideLoading('export data'));
      } finally {
        // turn off loading
        dispatch(hideLoading('export data'));
      }
  ...
  return (
    <div className={`${disabled ? 'printer-disabled' : 'printer-enabled'}`} tabIndex={0} onKeyDown={onKeydown}>
      <div onClick={actionPrint}>
