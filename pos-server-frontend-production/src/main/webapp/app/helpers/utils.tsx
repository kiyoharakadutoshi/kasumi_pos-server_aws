...
import {INAGEYA_CODE, LIMIT_RECORD_PAGE, LIST_COMPANY, MAX_LENGTH, STATUS} from 'app/constants/constants';
import JsPDF from 'jspdf';
import { isValid } from 'date-fns';
import { navigateScreen } from 'app/modules/menu-screen/navigate-screen';
...
export const parseBool = (value: number) => {
  return !!+value;
};
...
export const accessScreens = (permissions?: Permission[]) => {
  if (permissions?.length > 0) {
    return Object.keys(navigateScreen)
      .filter((key) => permissions.some((permission) => permission.code === key))
      .map((key) => navigateScreen[key]);
  }
...
export const formatValue = (code?: any, name?: string): string => {
  if (code && !isNullOrEmpty(name)) return `${code}：${name}`;
...
export const isInageyaCompany = (companyCode: string) => isEqual(companyCode, INAGEYA_CODE);
