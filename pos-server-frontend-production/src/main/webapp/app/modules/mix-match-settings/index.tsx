...
import {
  ...
  OptionUpdateMixMatch,
  postUpdateMixMatch,
} from 'app/services/sc05_mix_match_settings';
      ...
      .then((resp) => {
        ...
        const optionMapping = data.options.map((item: MixMatchDetailOption) => {
          return {
            ...
            priceCompleted: Number(item.value),
    ...
    const option = getValues('options')
      ...
      .map((item) => {
        return {
          ...
          value: Number(item.priceCompleted),
        } as OptionUpdateMixMatch;
      });
              ...
              <div className="mix-match__search-item">
                ...
                <div className="mix-match__search-item-value">
                  <div className="mix-match__promotion-period">
                    <DateTimeStartEnd
                      type="secondary"
                      onChangeStartDate={(date) => setValue('periodTimeMixMatch.startDate', convertDateServer(date))}
                      onChangeStartTime={(_time, timeStr?: string) => setValue('periodTimeMixMatch.startTime', timeStr)}
                      onChangeEndDate={(date) => setValue('periodTimeMixMatch.endDate', convertDateServer(date))}
                      onChangeEndTime={(_time, timeStr?: string) => setValue('periodTimeMixMatch.endTime', timeStr)}
                      startDate={getValues('periodTimeMixMatch').startDate}
                      endDate={getValues('periodTimeMixMatch').endDate}
                      startTime={getValues('periodTimeMixMatch').startTime}
                      endTime={getValues('periodTimeMixMatch').endTime}
