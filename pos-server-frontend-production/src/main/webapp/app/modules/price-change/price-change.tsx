...
import { GROUP_PRODUCT_CODE_INPUT_MAX_LENGTH, STATUS } from '@/constants/constants';
...
export const PriceChange = () => {
  ...
  const langSysterm = Intl.DateTimeFormat().resolvedOptions().locale.split('-')[0];
  ...
  const handleBlurMyCompanyCode = () => {
    ...
    dispatch(
      getHierarchyLevel({
        level: 1,
        store_code: getValues('selectedStored'),
      ...
      .then((res) => {
        ...
        if (hierarchyLevelData.total_count !== 0) {
          setValue('descriptionLevelOne', hierarchyLevelData?.items[0]?.description_level_one);
          setError('descriptionLevelOne', null);
        } else {
  ...
  const handlePrint = () => {
    dispatch(
      ...
      .then((res) => {
        ...
        const fileName =
          langSysterm === 'ja'
            ? `緊急売価変更リスト_` + selectedStores
            : `Emergency Price Change List_` + selectedStores;
        ...
        setTimeout(() => {
          if (enabledPLU) {
            ...
            const item = document.querySelector('input[name="listData[0].group_code"]');
            (item as HTMLInputElement)?.focus();
          }
        }, 50);
      })
                ...
                onClick={() => {
                  ...
                  setTimeout(() => {
                    const item = document.querySelector(
                      enabledPLU ? 'input[name="listData[0].group_code"]' : 'input[name="listData[0].item_code"]'
                    );
                    (item as HTMLInputElement)?.focus();
                  }, 50);
                }}
