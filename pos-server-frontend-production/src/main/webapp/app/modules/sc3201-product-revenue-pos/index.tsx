...
import { isNullOrEmpty, localizeString, isValidDate, getGroupProductCode } from 'app/helpers/utils';
...
const RevenueCheckByProductAndPOS = (): JSX.Element => {
  ...
  useEffect(() => {
    ...
    if (formDataRevenue?.tableData.length > 0) {
      ...
      setValue('selectedRows', formDataRevenue.selectedRows);
  ...
  const handleChangeOutputInitOption = (item: any) => {
    ...
    dispatch(getHierarchyLevel(payload))
      .unwrap()
      .then((res) => {
        ...
        switch (item.value) {
          case Level.LevelOne:
            fetchedUniteOptions = result.items.map((dataItem) => ({
              ...
              name: dataItem.description_level_one,
            }));
            break;
          case Level.LevelTwo:
            fetchedUniteOptions = result.items.map((dataItem) => ({
              ...
              name: dataItem.description_level_two,
            }));
            break;
          case Level.LevelThree:
            fetchedUniteOptions = result.items.map((dataItem) => ({
              ...
              name: dataItem.description_level_three,
            }));
            break;
          case Level.LevelFour:
            fetchedUniteOptions = result.items.map((dataItem) => ({
              ...
              name: dataItem.description_level_four,
            }));
            break;
          default:
            break;
