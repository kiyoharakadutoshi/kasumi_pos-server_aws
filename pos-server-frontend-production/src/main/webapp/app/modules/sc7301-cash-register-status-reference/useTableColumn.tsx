...
const UseTableColumn = () => {
  ...
  const renderRowDataMasterStatusAndApplyMasterTime = (row: {
    original: { cashRegisterStatus: number; dataMasterStatus: number; applyMasterTime: string; isHighlight: number };
  }) => {
    ...
    return (
      <div className={`update-date`}>
        ...
        <p className='text-update-master'>
          <span>{statusContent}</span>
          {row.original.isHighlight === 1 && <span className="text-red">(事故)</span>}
        </p>
      </div>
    );
  };
