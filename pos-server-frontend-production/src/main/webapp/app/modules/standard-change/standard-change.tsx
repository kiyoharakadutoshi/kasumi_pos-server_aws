...
import {
  focusElementByName, getGroupCode, getProductCode,
  isEqual,
  isEqualObject,
  isNullOrEmpty,
  localizeFormat,
...
import { getDetailHierachyLevel } from '@/services/hierarchy-level-service';
import {
...
import { compareDate, convertDateServer, getNextDate, isPastDate } from '@/helpers/date-utils';
...
export const StandardChange = () => {
  ...
  const suggestMycompanyCode = (value: string, index: number) => {
    ...
    // 2. Just enter the code, don't enter the group, there will be no group error, no suggestion (return)
    if (isNullOrEmpty(groupCode) && !isNullOrEmpty(productCode)) {
      return;
    }
  ...
  const focusInput = () => {
    setTimeout(() => {
      focusElementByName('searchCondition.bookingDate');
    }, 500);
  };
  ...
  const addRecordBlank = (existItemCount?: number) => {
    return Array.from({ length: 12 - (existItemCount ?? 0) }, (_, _index) => ({
      record_id: null,
      company_code: '',
      store_code: '',
      item_code: '',
  ...
  const handleBlurMyCompanyCode = (groupCode) => {
    ...
    dispatch(
      getDetailHierachyLevel({
        store_code: selectedStores[0],
        code_level_one: groupCode,
      })
    )
      .unwrap()
      .then((res) => {
        ...
        if (hierarchyLevelData) {
          descriptionLevelOne = hierarchyLevelData.description_level_one;
  ...
  const handleGetStandardPrice = ({ pageNumber }: { pageNumber?: number }, action: Action) => {
    ...
    dispatch(getStandardPriceHistory(requestParmGetStandardChange))
      ...
      .then((res) => {
        ...
        // ++ Action: Search (1)
        // If booking date is future date set standardPrices equal to dataDefault( 13 empty records)
        // If booking date is current date set standardPrices equal to [] ( 0 empty records)
        // ++ Action: Paging (2)
        // When paging, next will always set standardPrices equal to dataDefault( 13 empty records)
        if (res.data?.data?.total_item === 0) {
          switch (action) {
            case Action.Search:
              if (compareDate(bookingDate)) {
                // Mode add new
                setValue('standardPrices', dataDefault);
                setValue('standardPricesDefault', dataDefault);
                setTimeout(() => {
                  const item = document.querySelector('input[name="standardPrices[0].item_code"]');
        ...
        setValue(
          ...
          standardChanges.map((item) => ({
            ...item,
            product_code: item?.my_company_code?.length >= MAX_LENGTH.product_code ? getProductCode(item?.my_company_code) : '',
        ...
        setTimeout(() => {
          const item = document.querySelector('input[name="standardPrices[0].standard_price"]');
          (item as HTMLInputElement)?.focus();
        }, 50);
      })
      .catch(() => {});
  ...
  const handleClearData = () => {
    ...
    setTimeout(() => {
      const item = document.querySelector('input[name="searchCondition.bookingDate"]');
      (item as HTMLInputElement)?.focus();
    }, 50);
  };
          ...
          <div className={'standard-change__search'}>
            ...
            <div className={'standard-change-button'}>
              <ButtonPrimary
                ...
                onClick={() => setEnabledPLU(!enabledPLU)}
