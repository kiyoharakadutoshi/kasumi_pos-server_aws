...
const SizePanelCategory = ({
  ...
  const suggestCategory = (value?: string) => {
    if (!isNullOrEmpty(value) && value !== undefined) {
      ...
      dispatch(getHierarchyLevel({ store_code: presetButton.store_code, filter_code: value, level: 3 }))
        .unwrap()
        .then((response) => {
          const hierarchyLevel = response.data?.data?.items;
          if (hierarchyLevel?.length > 0) {
            onChangeDescription(hierarchyLevel[0].description_level_three);
          }
        })
        .catch(() => {});
    }
  ...
  const finishSearchCategory = (data?: IHierarchyLevelInfo) => {
    ...
    if (data) {
      onChangeCategory(data.code_level_three);
      onChangeDescription(data.description_level_three ?? '');
      dispatch(clearHierarchyMessageError());
    }
