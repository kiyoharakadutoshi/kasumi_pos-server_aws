...
import { addPreset, dragPreset, PresetState, selectButton, selectPreset, setButtonDescription, setErrorMessage, setPresetButtonProduct } from './reducer/preset-reducer';
import { GROUP_CODE_PRODUCTS, MAX_MENU_BUTTON_COLUMN, MAX_MENU_BUTTON_ROW, MAX_PRESET_TAB } from '@/constants/constants';
import { convertQueryStringToObject, getGroupCode, getProductCode, isNullOrEmpty, localizeString } from '@/helpers/utils';
...
const TableContent = () => {
  ...
  const [positionEmpties, setPositionEmpties] = useState<IPosition[]>([]);
  ...
  const createNewPositionEmpties = () => {
    const newPositions = new Set<string>();
    presetButtons?.forEach(button => {
      for (let i = button.button_row_number; i < button.button_row_number + button.button_row_span; i++) {
        for (let j = button.button_column_number; j < button.button_column_number + button.button_column_span; j++) {
          newPositions.add(`${i},${j}`);
        }
      }
    });
    const newPositionsEmpties: IPosition[] = [];
    for (let i = 1; i <= MAX_MENU_BUTTON_ROW; i++) {
      for (let j = 1; j <= MAX_MENU_BUTTON_COLUMN; j++) {
        if (!newPositions.has(`${i},${j}`)) {
          newPositionsEmpties.push({
            x: j,
            y: i,
          });
        }
    ...
    setPositionEmpties(newPositionsEmpties);
  };
  ...
  const handleSelectEmptyButton = (position: IPosition) => {
    handleSelectButton({
      ...initPreMenuButton,
      store_code: presetDetail?.store_code,
  ...
  const selectEmptyButtonKeydown = (event: React.KeyboardEvent<HTMLDivElement>, position: IPosition) => {
    if (event.key === ' ') {
  ...
  return (
    <div className={'body-content'}>
      ...
      >
        ...
        >
          {presetButtons?.map((item, idx) => {
            const heightImg = item.button_row_span > item.button_column_span ? (item.button_column_span / item.button_row_span) * 100 : 65;
            return (
              <div
                key={idx}
                className={`preset-button-container`}
                style={{
                  gridColumn: `${item.button_column_number} / span ${item.button_column_span}`,
                  gridRow: `${item.button_row_number} / span ${item.button_row_span}`,
                  aspectRatio: `${item.button_column_span} / ${item.button_row_span}`,
                }}
              >
                <PopoverText
                  onKeyDown={event => selectPresetButtonKeydown(event, item)}
                  tabIndex={0}
                  className={`preset-button preset-button-click ${item.button_column_number === presetReducer?.presetButton?.button_column_number &&
                    item.button_row_number === presetReducer?.presetButton?.button_row_number
                    ? 'selected-item'
                    : 'normal-item'
                    }`}
                  style={{
                    backgroundColor: `${colorButtons.find(color => color.type === item.style_key).color}`,
                    opacity: item.display_status === 0 ? 0.5 : 1,
                  }}
                  onClick={() => handleSelectButton(item)}
                  text={item.description}
                  valueChangePopover={item}
                  height={item.style_info?.length > 0 ? `${98 - heightImg}%` : '100%'}
                  alignItems="start"
                >
                  {item.style_info?.length > 0 && (
                    <div className="item-img" style={{ height: `${heightImg}%` }}>
                      <img src={item.style_info} alt="Icon" />
                    </div>
                  )}
                </PopoverText>
              </div>
          ...
          {positionEmpties?.map((item, idx) => (
            <div key={idx} className={`preset-button-container`}>
              <div
                ...
                className={`preset-button-empty preset-button-click ${item.x === presetReducer?.presetButton?.button_column_number && item.y === presetReducer?.presetButton?.button_row_number
                  ? 'selected-item'
