...
import { storeRes, UserTable } from '../interface-user';
...
export const ModalUser: React.FC<DefaultModalProps> = ({
  ...
  const isEqualStoreList = (array1: any, array2: IDropDownItem[]): boolean => {
    let values1: (string | number)[] = [];
    let values2: (string | number)[] = [];
    if (typeof array1 === 'string') {
      values1.push(array1);
    } else {
      values1 = array1?.map((item: { value: string | number; }) => item?.value);
    }

    if (typeof array2 === 'string') {
      values2.push(array2);
    } else {
      values2 = array2?.map((item) => item?.value);
    }
    return arraysEqual(values1, values2);
  }
  ...
  const disableUpdate = useMemo(() => {
    ...
    return (
      ...
      isNullOrEmpty(item?.password) &&
      isEqualStoreList(item?.stores, dataEditDefault?.stores)
    );
  }, [watch('edit.userId'), watch('edit.userName'), watch('edit.role'), watch('edit.password'), watch('edit.stores')]);
  ...
  const handleUpdateUser = async () => {
    ...
    if (selectedRole === 2 && isNullOrEmpty(edit?.stores)) {
      form.setError(`${modeStatus}.stores`, { message: localizeFormat('MSG_VAL_001', 'modalUser.label.store') });
      isError = true;
    }
    ...
    let selectedStoreList: storeRes[];

    if (!Array.isArray(edit?.stores)) {
      const storeNameTemp = LIST_STORE.find(
        (item) => item.value === edit?.stores
      )?.name;
      selectedStoreList = [{
        store_code: edit?.stores,
        store_name: storeNameTemp
      }]
    } else {
      selectedStoreList = (edit.stores as IDropDownItem[])?.map((item) => ({
    ...
    const store = (userDefault?.stores || [])?.map((item) => ({
    ...
    const isNoChange = isEdit &&
      ...
      isEqualStoreList(store, edit?.stores)
  ...
  const handleCreateUser = async () => {
    ...
    if (selectedRole === 2 && isNullOrEmpty(add?.stores)) {
      form.setError(`${modeStatus}.stores`, { message: localizeFormat('MSG_VAL_001', 'modalUser.label.store') });
      isError = true;
    }
    ...
    if (!Array.isArray(add?.stores)) {
      const storeNameTemp = LIST_STORE.find(
        (item) => item.value === add?.stores
      )?.name;
      selectedStoreList = [{
        store_code: add?.stores,
        store_name: storeNameTemp
      }]
    } else {
      selectedStoreList = (add.stores as IDropDownItem[])?.map((item) => ({
        store_code: item.code,
        store_name: item.name,
      })) as storeRes[]
    }
  ...
  return (
    ...
    >
      ...
      <SelectControl
        ...
        name={`${modeStatus}.stores`}
        items={LIST_STORE || []}
        value={getValues(`${modeStatus}.stores`)?.[0]?.value || getValues(`${modeStatus}.stores`)}
        isRequired
        hasBlankItem={true}
